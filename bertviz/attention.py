	...
	if not is_sentence_pair:  # Single sentence
		if model_type in ('bert', 'roberta'):
			...
			tokens_a = [tokenizer.cls_token] + tokenizer.tokenize(sentence_a) + [
				tokenizer.sep_token] + [tokenizer._mlb_token] + [tokenizer.sep_token]
		...
		if model_type == 'bert':
			...
			tokens_a = [tokenizer.cls_token] + tokenizer.tokenize(sentence_a) + [
				tokenizer.sep_token]
			tokens_b = tokenizer.tokenize(sentence_b) + [tokenizer.sep_token] + [tokenizer._mlb_token] + [tokenizer.sep_token]
			token_type_ids =  torch.LongTensor([[0] * len(tokens_a) + [1] * len(tokens_b)])
